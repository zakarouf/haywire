...
{
    hw_FnObj *fnobj = HW_THREAD_ALLOC(hw, sizeof(*fnobj));
    HW_ARR_NEW(hw, fnobj->code, 32);
    HW_ARR_NEW(hw, fnobj->vinfo, 32);
    
    fnobj->lables = hw_SymTableOrd_new(hw, 16);
    fnobj->knst = hw_SymTableOrd_new(hw, 16);
    fnobj->parent_mod = mobj;
...
{
    hw_SymTableOrd_delete(hw, fnobj->lables);
    hw_SymTableOrd_delete(hw, fnobj->knst);
    HW_ARR_DELETE(hw, fnobj->vinfo);
    HW_ARR_DELETE(hw, fnobj->code);
    HW_THREAD_FREE(hw, fnobj);
}
...
{
    hw_SymTableOrd_set(hw, fnobj->knst, name, name_sz, value, tid);
}

void hw_FnObj_lable(hw_State *hw, hw_FnObj *fnobj
    , hw_byte const *name, hw_u32 name_sz)
{
    hw_SymTableOrd_set(hw, fnobj->lables, name, name_sz
        , (hw_Var){.as_uint = fnobj->code->lenUsed}, hw_TypeID_uint);
}
